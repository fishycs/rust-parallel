...
pub fn parallel_prefix_sum<T: Send>(array: &mut[T]) where for<'t> T: AddAssign<&'t T> {
    parallel_prefix_sum_tune(array, num_cpus::get(), SEQUENTIAL_THRESHOLD);
}

// parallel prefix sum (copy)
pub fn parallel_prefix_sum_tune<T: Send>(array: &mut[T], cores: usize, threshold: usize) where for<'t> T: AddAssign<&'t T> {
    parallel_prefix_tune(array, &|a: &mut T, b: &T| {
	*a += b;
    }, cores, threshold);
}
...
pub fn parallel_prefix_tune<T: Send, F: Fn(&mut T, &T) + Sync>(array: &mut[T], accumulate_fn: &F, cores: usize, threshold: usize) {
...
// collect and build sums
fn parallel_prefix_accumulate<T: Send, F: Fn(&mut T, &T) + Sync>(array: &mut[T], accumulate_fn: &F, cores: usize, threshold: usize) {
...
// finalize and distribute sums
fn parallel_prefix_distribute<T: Send, F: Fn(&mut T, &T) + Sync>(array: &mut[T], accumulate_fn: &F, cores: usize, threshold: usize, start: bool) {
    if cores == 1 || array.len() <= threshold {
	// sequential
